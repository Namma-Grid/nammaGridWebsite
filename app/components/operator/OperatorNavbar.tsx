...
const NAV = [
  ...
  { href: '/operator/demand', label: 'Demand & Grid', icon: '📊' },
  { href: '/operator/infrastructure', label: 'Infrastructure', icon: '🏗️' },
  { href: '/operator/schedule', label: 'Schedule Optimizer', icon: '⏱️' },
];
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'glass-nav shadow-sm' : 'bg-white/60 backdrop-blur-sm'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between">
          ...
          <Link href="/operator" className="flex items-center gap-2 group">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-white text-sm font-bold shadow-md">
              ⚡
            </div>
            <div className="flex flex-col">
              ...
              <span className="text-[9px] text-blue-500 font-semibold uppercase tracking-widest leading-none">Operator</span>
            </div>
          </Link>
        ...
        <div className="mobile-menu-panel md:hidden">
          <div className="p-6 pt-8 space-y-1">
            <div className="flex items-center justify-between mb-6">
              <span className="text-base font-bold text-slate-800">Operator Menu</span>
            ...
            {NAV.map((item) => (
              <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-base font-medium transition-all ${pathname === item.href ? 'text-blue-700 bg-blue-50' : 'text-slate-600 hover:bg-slate-50'}`}>
